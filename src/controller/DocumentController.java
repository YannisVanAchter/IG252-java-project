...
public class DocumentController {
     ...
     */
    public String[] getAllDocumentType() throws DataValidationException {
        return Stream.concat(
                Stream.of("All"),
                getAllDocuments().stream()
                        .map(doc -> doc.getDocumentType().getName())
        ).toArray(String[]::new);
    }

    /**
     * Redondance à supprimer. En attente de Tostring dans ClientSupplier pour remplacer par
     * @see #getAllClientSupplier().toString implicite
     */
    public String[] getAllClientNames() throws DataValidationException {
        return getAllClientSupplier().stream()
                .map(ClientSupplier::getName)
                .toArray(String[]::new);
    }
     ...
     * @throws DataValidationException à supprimer (présent à cause de la creation dans getAllDocuments())
     */
    public String[] getAllWorkflowStatus() throws DataValidationException {
        return getAllDocuments().stream()
                .map(wf -> wf.getWorkflow().getStatus().getName())
                .toArray(String[]::new);
    }
