...
public class DiscountDA extends CRUD<Discount> {
    ...
    private Map<Integer, Discount> dataMappingObject;
    ...
    @Override
    public Discount mapDataToObject(ResultSet data, boolean mapping) throws DataBaseException {
        int id;

        try {
            int quantity = data.getInt("requiredQuantity");
            BigDecimal discountPercentage = data.getBigDecimal("discountPercentage");
            LocalDate startDate = SQLDateToLocalDate(data.getDate("startDate"));
            LocalDate endDate = SQLDateToLocalDate(data.getDate("endDate"));
    ...
    @Override
    public List<Discount> getAll() throws DataBaseException {
     ...
     */
    @Override
    public List<Discount> getsByIds(List<Integer> hashCodes, boolean mapping) throws DataBaseException {
        return getAll().streams().filter(d -> 
                hashCodes.contains(Discount.hashCodes(d.getRequiredQuantity(), d.getDiscountPercentage(), d.getStartDate(), d.getEndDate()))
            );
    }

    @Override
    public List<Discount> getsByIds(List<Integer> ids) throws DataBaseException {
        return getsByIds(ids, true);
    }

    @Override
    public Discount getById(int id, boolean mapping) throws DataBaseException {
        getAll();
    ...
    @Override
    public Discount getById(int id) throws DataBaseException {
        return getById(id, true);
    }

    @Override
    public boolean insert(Discount newDiscount) throws DataBaseException {
    ...
    public boolean update(Discount discount, Discount newDiscount) throws DataBaseException {
        ...
        try (Connection connection = MySQLConnector.getInstance().getConnection()) {
            ...
            dataMappingObject.remove(discount.getDiscountId());
            dataMappingObject.put(discount.getDiscountId(), discount);
    ...
    @Override
    public boolean checkExist(Discount discount) throws DataBaseException {
        boolean exist = false;
